...
import { FAQservice } from "../data";
import { Link, useNavigate } from "react-router-dom";

const InfoCar = () => {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const [data, setData] = useState(FAQservice);
  ...
  useEffect(() => {
    const debounce = setTimeout(() => {
      const filteredService = filterService(search, FAQservice);
      ...
      <div className=" w-[270px] relative flex flex-col items-center">
        ...
        <div className="mt-8 flex flex-col gap-5">
          ...
          {data.map((el, i) => (
            <button
              key={i}
              className="w-[270px] px-3 text-main-white text-base text-center font-bold bg-[linear-gradient(45deg,_#264404_29%,_#396904_100%)] hover:bg-[linear-gradient(45deg,_#264404_29%,_#396904_70%)] py-3 rounded-2xl "
            >
              {" "}
